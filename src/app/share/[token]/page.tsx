...
export default async function SharedReportPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const saved = await prisma.report.findUnique({ where: { shareToken: token }, select: { analysis: true, llm: true, participantA: true, participantB: true, user: { select: { reportName: true, name: true } } } });
